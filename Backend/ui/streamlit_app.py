...
backend_core_path = os.path.join(os.path.dirname(__file__), '..', 'core')
backend_core_path = os.path.abspath(backend_core_path)
sys.path.insert(0, backend_core_path)

# Import backend modules
from autonomous_repair import autonomous_repair
from dsl_parser import parse_dsl_config, validate_config, is_rule_allowed
from sandbox_runner import run_in_sandbox
from error_parser import parse_error
from logical_validator import validate_logic
...
st.markdown("""
<style>
    /* === FORCE DARK THEME - HIDE THEME TOGGLE === */
    /* Hide the theme toggle button in settings */
    button[kind="header"] {
        display: none !important;
    }
    
    /* Hide theme toggle in hamburger menu */
    [data-testid="stSidebarNav"] button[aria-label*="theme"],
    [data-testid="stSidebarNav"] button[aria-label*="Theme"] {
        display: none !important;
    }
